...
TABLES_TO_EXPORT = [
    t.strip()
    for t in os.environ.get("TABLES_TO_EXPORT", "analysis_results,prices,raw_news").split(",")
]
...
def _export_table(bq_client, storage_client, bucket, run_id: str, table_name: str, export_dir: str) -> None:
    ...
    _retry(lambda: bq_client.extract_table(
        table_ref, destination_uri, job_config=job_config, location=BQ_LOCATION
    ).result())
    ...
    for i, blob in enumerate(data_blobs):
        local_name = f"{table_name}{ext}" if len(data_blobs) == 1 else f"{table_name}_{i}{ext}"
...
def export_bq_to_kaggle(request):
    ...
    try:
        ...
        for table_name in TABLES_TO_EXPORT:
            ...
            _export_table(bq_client, storage_client, bucket, run_id, table_name, export_dir)
